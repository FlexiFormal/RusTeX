...
Another aspect of interoperability in a mathematical virtual research environment -- as is the goal of \ODK\ -- is the possibility
of distributed multisystem computations, where e.g. a given system
...
in the \ODK project (see \autoref{fig:interop}), which are based on
...
in $A$. For example, the method call \expr{h.cardinality()} on a
\Sage handle \texttt{h} to a \GAP object \texttt{G} should trigger in
\GAP the corresponding function call \expr{Size(G)}.
...
certain types of objects, like \Sage's \expr{PermutationGroup} or
\expr{MatrixGroup}. However, this implementation lacks modularity:
for example, if \texttt{h} is a handle to a mere set \texttt{S}, \Sage
cannot use the \emph{adapter method} that maps
\expr{h.cardinality()} to \expr{Size(S)}, because this adapter
method is only available in the above two adapter classes.

To get around this problem, Nicolas M. Thi{\'e}ry and others have worked on a more semantic
...
available level of generality, as in \autoref{lst:adapter}.
...
systems. This is where the Math-in-the-Middle paradigm comes to the rescue. With it,
...
generated.
%
In our example the adapter method for \expr{cardinality} can be
constructed automatically as soon as the MitM interface views link the
\expr{cardinality} function in the \Sage interface theory on Sets
with the \expr{Size} function in the corresponding interface theory
for \GAP.
...
\paragraph{Exporting the \GAP Knowledge: Type System Documentation}
\label{sec:gaptypes}

The \GAP type system encodes a wealth of mathematical
...
specified independently for each method (the details are discussed later).
%
...
\begin{figure}%r{6cm}%\vspace*{-2em}
  \begin{center}\includegraphics[height=\textheight]{img/odk-gap-graph.pdf}\vspace*{-.5em}\end{center}
...
As a first step in generating interface theories for the MitM ontology, \GAP developers have implemented
...
visualization and exploration. The JSON output of the \GAP object system with default packages is currently
...
in \autoref{sec:lmfdb}. This conversion is currently under development and will lead
to a significant increase of the scope of the MitM ontology. 

\begin{figure}[ht]\centering
  \includegraphics[width=\textwidth]{img/odk-gap-ismagma}
  \caption{The \GAP Knowledge Graph (fragment).\label{fig:gap-ismagma}}
\end{figure}

As a side-effect of this work, Markus Pfeiffer discovered quite a few inconsistencies in the \GAP
documentation, which came from a semi-automated conversion of GAP manuals from the
\TeX-based manuals used in GAP 4.4.12 and earlier.  In response, he implemented a consistency checker
...
eliminated in a subsequent cleanup.

\paragraph{Semantics in the \Sage Category System}
...
%   Size/cardinality}
\begin{figure}\centering%r{8cm}\vspace*{-2.5em}
...
\end{figure}
To generate interface theories from the \Sage category system, Nicolas Thiéry et al are experimenting with a
system of annotations in the \Sage source files. Consider for instance the situtation in
\autoref{fig:anncat} where we have annotated the \expr{Sets()} category in \Sage
with \expr{@semantic} lines that state correspondences to other interface theories. From
