...
The Math-in-the-Middle architecture is used heavily in the \ODK project. It centers around the Math-in-the-Middle \emph{ontology}, an \mmt archive of formalized math\footnote{Available at \url{https://gl.mathhub.info/MitM/smglom} and \url{https://gl.mathhub.info/MitM/Foundation}} using all of the features introduced in \autoref{ch:lf}. In particular, this archive provides useful real-world examples for these features. The ontology serves as a mediator between individual systems involved in the \ODK project to facilitate knowledge exchange and remote procedure calls.

Additionally, the Math-in-the-Middle architecture can be used in other contexts. In particular, it facilitates across-library knowledge management services for formal libraries, as described in \autoref{ch:crosslib}. From this perspective, while this section focuses on the role of the architecture in the \ODK project, the general ideas underlying this section can be generalized beyond the systems involved in \ODK.

Since we aim to make our components interoperable at a mathematical level, we have to establish a common meaning space that will 
...
formalization that translates into direct gains in interoperability. For this reason, neither
...
\begin{figure}[ht]\centering
  \begin{tabular}{|c|c|c|}\hline
    ...
    \input{img/odk-fullgraph} & \input{img/odk-mstargraph} & \input{img/odk-stargraph}\\\hline
...
The descriptions in the MitM ontology must simultaneously be system-near to make
...
circles in \autoref{fig:interface-theories}) that are at the same time system-specific
...
\autoref{fig:interface-theories}) as they are connected to the core MitM ontology (the
...
\begin{figure}[ht]%r{4cm}%\vspace*{-1.5em}
	\centering
  \input{img/odk-mistargraph}\vspace*{-.5em}
  \caption{Interface Theories}\label{fig:interface-theories}\vspace*{-1em}
\end{figure}


Apart from \ODK, the MitM ontology is furthermore used in the FrameIT~\cite{RocKohMue:frsmgmmo16} approach
for developing serious games for math education, and the MaMoRed project (\cite{KohKopMue:mmrdftg17},\cite{KopKohTab:mpdrmm18}) with the goal of building a modular library of computational models in physics and related STEM fields.

% \subsection{Realizing and utilizing a MitM ontology}

%\begin{wrapfigure}r{6.2cm}\centering\vspace*{-2em}
%  \input{img/odk-metalogics}\vspace*{-.5em}
%  \caption{A OMDoc/MMT Theory Graph}\label{fig:mmt}\vspace*{-1em}
%  \label{figure.omdoc.example}
%\end{wrapfigure}

% Naturally, our current candidate for representing the MitM ontology is the \ommt format.
...
%\autoref{fig:mitm}, where we drill into the MitM information architecture from
%\autoref{fig:interface-theories}, but restrict at this stage to three systems from the \ODK
